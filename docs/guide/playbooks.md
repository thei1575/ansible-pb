...
Every `*.yml` in `playbooks/` becomes a row. The description column is the
file's own header comment where it has one, and the first play's `name`
otherwise — so a playbook documents itself in the list by being commented.

The pane on the right always shows the exact `ansible-playbook` command your
current options produce, and the hosts it would touch. Nothing runs that you
have not read first.
...
about to change", so pb first asks Ansible for the real host list with
